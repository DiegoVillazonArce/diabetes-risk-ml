...
Phases P0-P10 are complete, including public deployment, probability-quality work, SHAP explainability, and the constrained model scenario explorer. The frozen D-016 `HistGradientBoostingClassifier` remains the selected model; D-018 accepted `calibration_method = none`, so the schema-version-2 artifact serves the model's positive-class probability without a post-hoc calibrator; and D-019 retains a probability-only product with no decision threshold or high/low-risk label. P9 explains that exact probability with SHAP 0.52.0 without changing the estimate. P10 compares the submitted estimate with one hypothetical `PhysActivity`, `Fruits`, or `Veggies` input and describes only model sensitivity, not a real health effect. D-023 through D-025 are Accepted, and US-0605, US-0610, and US-0611 are Done. Implementation commit `fb50ed9` was pushed and the updated Streamlit app passed mandatory public frontend verification on 2026-07-15, preserving the P8/P9 contracts, approved whitelist, neutral scenario wording, medical disclaimer, and artifact integrity. P11 is now refined and **Ready**: US-0603, US-0612, and US-0613 define a privacy-safe in-memory CSV template, validation, batch-scoring, download, and public-verification workflow, while D-026 through D-028 remain Pending until their ordered implementation evidence exists. The public app does not yet include batch prediction. P12-P13 remain Future.
...
The app loads the local artifact and the separate privacy-safe SHAP background asset once, renders the 21-feature input form, and shows the schema-version-2 contract's positive-class probability as an educational risk percentage with a visible medical disclaimer. After a valid prediction, P9 adds a compact explanation of the largest factors that increased or decreased the model's estimate relative to its reference estimate. The P10 explorer then offers one neutral hypothetical comparison over the D-023 whitelist (`PhysActivity`, `Fruits`, or `Veggies`), shows original and hypothetical probabilities plus the signed percentage-point difference, identifies the effective change, and provides an exact reset. It does not calculate SHAP for the scenario or present the comparison as causal, clinical, or recommended. The current D-018 outcome is `none`, so the UI still identifies the probability as uncalibrated; D-019 still adds no decision threshold or high/low-risk label. Streamlit does not read the project training CSV, derive the background, train or calibrate a model, search scenarios, or run global analysis. It creates and caches the local `TreeExplainer` from the versioned aggregate asset; widget values and the latest validated baseline remain transient within the active session, the baseline is bound to the exact model-artifact SHA-256, and any failed resubmission or hash change invalidates it before a result renders. Project code neither writes nor externally logs that state. P11 plans a separate in-memory user-uploaded CSV path, but that workflow is not implemented or deployed yet.
...
The app is deployed on Streamlit Community Cloud at [https://brfss-diabetes-risk-estimator.streamlit.app/](https://brfss-diabetes-risk-estimator.streamlit.app/). It uses branch `main`, entry point `app/streamlit_app.py`, Python 3.12, the pinned `requirements.txt`, and the D-013 version-controlled artifact. The public P9 implementation was introduced by commit `25c4ed4`, and the P10 scenario explorer by commit `fb50ed9`; both continue serving the schema-version-2 P8 artifact introduced by `5798a0e`. Public P10 verification on 2026-07-15 confirmed startup, prediction, the original P9 explanation, progressive scenario disclosure, the exact three-field whitelist, effective-change and reset behavior, neutral model-sensitivity wording, and the medical disclaimer, with no browser-console errors on the independently reviewed healthy path. The user also confirmed the planned frontend case set. Streamlit does not train or calibrate at runtime, download a model, read the raw CSV, derive the SHAP background, run global explanation analysis, optimize scenarios, or persist submitted profiles.
