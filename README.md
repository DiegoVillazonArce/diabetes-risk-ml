...
Phases P0-P10 are complete, including public deployment, probability-quality work, SHAP explainability, and the constrained model scenario explorer. The frozen D-016 `HistGradientBoostingClassifier` remains the selected model; D-018 accepted `calibration_method = none`, so the schema-version-2 artifact serves the model's positive-class probability without a post-hoc calibrator; and D-019 retains a probability-only product with no decision threshold or high/low-risk label. P9 explains that exact probability without changing it, and P10 compares one approved hypothetical input using model-sensitivity wording only. P11 now has a complete **local implementation ready for review**: D-026, D-027, and D-028 are Accepted; US-0603 and US-0612 are Done; and US-0613 is In Progress because commit/push, deployment, and mandatory public valid-plus-mixed CSV verification remain external work. The local app offers an in-memory generated template/field guide, strict structural and row validation, partial-success vectorized scoring, bounded preview, and deterministic safe download while preserving P8-P10. Roadmap P11 remains **Ready**, not Done, and the public app still does **not** include P11 batch prediction. P12-P13 remain Future.
...
The app is deployed on Streamlit Community Cloud at [https://brfss-diabetes-risk-estimator.streamlit.app/](https://brfss-diabetes-risk-estimator.streamlit.app/). It uses branch `main`, entry point `app/streamlit_app.py`, Python 3.12, the pinned `requirements.txt`, and the D-013 version-controlled artifact. The public deployment currently includes P8-P10 only: P9 was introduced by commit `25c4ed4`, and P10 by `fb50ed9`; public P10 verification passed on 2026-07-15. The unstaged P11 implementation in this working tree has not been committed, pushed, deployed, or publicly smoke-tested, so the public URL does not yet provide batch prediction. Streamlit does not train or calibrate at runtime, download a model, read the raw CSV, derive the SHAP background, run global explanation analysis, optimize scenarios, or persist submitted profiles.
