...
[![CI](https://github.com/DiegoVillazonArce/diabetes-risk-ml/actions/workflows/ci.yml/badge.svg)](https://github.com/DiegoVillazonArce/diabetes-risk-ml/actions/workflows/ci.yml)
...
**Live demo.** [brfss-diabetes-risk-estimator.streamlit.app](https://brfss-diabetes-risk-estimator.streamlit.app/) serves the completed P0–P13 portfolio baseline.
...
Phases P0-P13 are complete as of 2026-07-20. The portfolio includes the public probability estimator, SHAP explanations, the constrained scenario explorer, privacy-safe batch scoring, the responsible offline fairness audit, accessible product/architecture communication, synthetic-only demo assets, and clean-clone CI. The frozen D-016 `HistGradientBoostingClassifier` remains the selected model; D-018 accepted `calibration_method = none`; and D-019 retains a probability-only product with no decision threshold or high/low-risk label. P13 decisions D-032 through D-035 are Accepted, implementation commit `7f642af` and cross-platform CI fix `08170c3` were pushed, GitHub Actions passed remotely, and the Streamlit deployment passed the public individual/explanation/scenario, valid-plus-mixed batch/download, navigation/About, link, disclaimer, and responsive-layout checks. The [architecture](docs/architecture.md), synthetic-only [portfolio assets](docs/p13-portfolio/), and tiered [narratives](docs/portfolio-summary.md) form the completed portfolio package. Both official artifacts and the four reference displays remain unchanged; the P12 audit remains descriptive and does not certify fairness.
...
The app is deployed on Streamlit Community Cloud at [https://brfss-diabetes-risk-estimator.streamlit.app/](https://brfss-diabetes-risk-estimator.streamlit.app/). It uses branch `main`, entry point `app/streamlit_app.py`, Python 3.12, the pinned `requirements.txt`, and the D-013 version-controlled artifact. P9 was introduced by commit `25c4ed4`, P10 by `fb50ed9`, P11 batch prediction by `246d5ff`, and P13 product polish by `7f642af` plus cross-platform CI fix `08170c3`. Public P13 verification on 2026-07-20 covered the healthy individual prediction/explanation/scenario path, valid-plus-mixed batch workflow and downloads, three-section navigation/About links, disclaimer, and responsive presentation. Streamlit does not train or calibrate at runtime, download a model, read the raw project CSV, derive the SHAP background, run global explanation analysis, optimize scenarios, persist uploaded profiles or results, or externally log user content.
