...
- [x] Confirm locally that the app resolves and loads only the artifact and provenance defined by D-013, and rejects absent, corrupt, incompatible, or untrusted bundles as required by the selected policy with clear guidance. The app loads only `models/diabetes_risk_model.joblib` through the hardened validator; missing/corrupt/incompatible bundles fail with regeneration guidance, artifact provenance and runtime versions are enforced against the P7 pins, and the official artifact's absence now fails rather than skips the deployment-reference tests. Deployment-environment confirmation remains explicitly pending in US-0703 and the end-to-end task below.
...
- [ ] Verify the implemented policy end to end from both a clean clone and the actual deployment environment. Clean-environment half done on 2026-07-11 (committed-artifact journey and full-training journey both pass; see US-0701 evidence); the deployment-environment half is pending until the authorized commit/push and Community Cloud deployment.

#### US-0703: Public Deployment

- [ ] Verify the current app and artifact path contract on a Linux, Python 3.12, Streamlit Community Cloud-compatible environment, including case-sensitive paths and repository-root execution assumptions. Static checks passed on 2026-07-11 (all project paths are `pathlib`-based with forward slashes, tracked filenames match the code's casing exactly, absolute paths derive from the module location so the platform's repository-root `streamlit run` is compatible); execution on the Linux platform itself is pending.
...
- [ ] Configure the Streamlit Community Cloud deployment from the correct GitHub repository, branch, and entry point, and select Python 3.12 in the platform settings. Pending: blocked on authorization to commit/push and on authenticated platform access. Planned configuration: repository `DiegoVillazonArce/diabetes-risk-ml`, branch `main`, entry point `app/streamlit_app.py`, Python 3.12 (the platform default).
...
- [ ] Verify boundary and representative exact ages map to the correct BRFSS age-group codes before inference. The committed reference profiles pin boundary ages 24, 65, 70, and 80 to codes 1, 10, 11, and 13 and pass locally, including through the headless form; the public-app repetition is pending.
- [x] Define and locally verify a small deployment-smoke reference set before public verification: record every profile's exact 21-feature inputs, derive its expected displayed probability locally from the official artifact, and document an appropriate comparison tolerance or display-rounding rule without changing the model or threshold to force a match. The manually observed P6 outputs near 0.3%, 60%, 70%, and 79.9% guided profile selection but became established fixtures only after their inputs and expectations were recorded and verified in P7. Defined in `tests/reference_profiles.py` (committed-ready; the commit itself awaits authorization): four profiles with exact 21-feature inputs, exact UI ages with their BRFSS codes, official-artifact probabilities, expected displays (0.3%, 60.0%, 70.0%, 79.9%), a documented +/-0.0002 probability tolerance with a 0.00025 display-rounding margin, and 31 tests in `tests/test_reference_profiles.py` covering the contract, mandatory artifact recomputation, an anti-tampering and pinned-provenance check of the D-016 artifact, and end-to-end headless form runs; `python -m tests.reference_profiles` prints the manual public checklist. Committing the set and exercising those same profiles on the public app remain covered by the unchecked commit/deployment and public-smoke tasks.
- [ ] Verify clear user-facing errors for an absent or invalid artifact using a controlled deployment check without leaving the public app broken. Local equivalents pass (missing and corrupt artifacts show the regeneration guidance in the headless app); the controlled public check is pending.
- [ ] Confirm the educational/medical disclaimer remains visible alongside prediction output on the public deployment. Verified locally in every headless run; public confirmation pending.
- [ ] Confirm the raw CSV, secrets, and submitted user inputs are not published, logged by project code, or persisted. Locally verified: no tracked CSV or secrets, no logging/persistence of inputs in project code, and the local-only loading guard passes; public confirmation pending.
