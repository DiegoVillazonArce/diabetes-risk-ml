...
P11 is a delivery extension over the unchanged schema-version-2 P8 probability contract. It does not create a second model, calibrator, decision layer, explanation contract, or scenario engine. The current app and source guards prohibit access to the project training CSV and disk persistence; P11 must preserve those guarantees while distinguishing them from a bounded user-uploaded CSV parsed only in memory. The current one-case helpers validate a mapping and score one row, so P11 needs a pure batch boundary that reuses their executable sources of truth without looping through UI code or duplicating feature rules.

The planning candidates are a UTF-8 comma-delimited file of at most 2 MiB and 1,000 data rows, exactly the 21 technical feature names in any input order, canonical reordering to `FEATURE_COLUMNS`, numeric codes documented by a generated field guide, BRFSS group codes `1`-`13` for `Age`, and no target, exported index, identifier, free-text, or passthrough column. A structurally invalid file would be rejected as a whole; within a structurally valid file, valid rows would be scored while invalid rows receive complete validation errors and no probability. These are candidates only: D-026 through D-028 stay Pending until the ordered spike, pure-engine evidence, and UX/privacy/performance review accept or replace them before dependent work.

#### Increment 1 -- Input, Validation, and Resource Contract (approximately 1 day)

- [ ] Inventory the exact batch-facing sources of truth: `FEATURE_COLUMNS`, `VALUE_RANGES`, `feature_label`, `format_feature_value`, schema-version-2 artifact validation, and the D-018/D-019 serving policy. Document where batch behavior may adapt presentation without changing model inputs.
- [ ] Spike CSV parsing from bytes with UTF-8 and optional UTF-8 BOM, comma delimiter, one header, duplicate-header detection, empty/malformed-file handling, and deterministic differentiation between structural errors and row errors. Do not accept spreadsheet, URL, ZIP, JSON, or remote inputs.
- [ ] Evaluate and record the proposed 2 MiB/1,000-row limits, exact-column/any-order policy, `Age` representation, identifier exclusion, numeric-code requirement, and code-generated template/field-guide design. Resolve D-026 before the parser or template contract is implemented.
- [ ] Prototype complete per-row validation without silent fill, rounding, clipping, truthy conversion, type coercion, or dropping duplicates. Resolve D-027 with file rejection, partial-success, output schema, probability precision, error ordering, and CSV serialization rules before Streamlit integration.

#### Increment 2 -- Pure Batch Validation and Scoring Engine (approximately 1-2 days)

- [ ] Implement a Streamlit-independent module, expected at `src/batch.py`, that accepts uploaded bytes plus a validated bundle and returns immutable/independent structured validation and scoring results.
- [ ] Reject empty, malformed, wrong-encoding/delimiter, headerless, duplicate-header, missing-column, unexpected-column, target-bearing, and over-limit files before scoring. Preserve every structurally valid row and its original order, including exact duplicate profiles.
- [ ] Validate all 21 values per row against integer-like, finite `VALUE_RANGES` rules and return deterministic complete error information. Never silently repair an invalid value and never score an invalid row.
- [ ] Score valid rows through one vectorized call to the same validated P8 scorer selected by the artifact, and prove each probability equals the existing single-case helper within absolute tolerance `1e-12`.
- [ ] Generate the template, field guide, combined result table, and UTF-8 CSV download entirely in memory with stable canonical column/error ordering. Resolve any implementation-dependent part of D-027 from focused evidence before UI work.

#### Increment 3 -- Streamlit Batch Delivery (approximately 1-2 days)

- [ ] Perform the D-028 UX/privacy/failure/performance review before changing Streamlit: navigation relative to the single-case form, explicit processing action, bounded preview, summary counts, safe error visibility, reset/replacement behavior, download naming, transient state, and medical/privacy wording.
- [ ] Measure the accepted maximum valid batch on the official artifact. Evaluate the proposed warm bound of 2 seconds and 50 MiB incremental Python memory; accept or replace both limits in D-028 before full integration and do not relax them after observing the final UI run.
- [ ] Integrate template/field-guide download, upload, validation summary, bounded preview, and result download without putting pandas CSV parsing or serialization logic in `app/streamlit_app.py`. Bind retained batch results to both the artifact SHA-256 and upload SHA-256, and invalidate them after replacement, failure, or artifact change.
- [ ] Keep uploaded bytes and generated results only in active-session memory. Add no filesystem/object-store/database write, analytics, external logging, URL fetch, or cross-session cache of user content.
- [ ] Keep P9 explanations and P10 scenarios exclusively in the single-case workflow. Batch output contains probabilities and validation status only, with D-019 wording and the medical disclaimer visible.

#### Increment 4 -- Regression, Deployment, and Closure (approximately 1 day)

- [ ] Run focused batch tests, app/headless tests, the complete suite, dependency validation, whitespace checks, and local browser review with valid, invalid, mixed, duplicate, maximum-size, reset/replacement, and download cases.
- [ ] Confirm the four reference profiles produce unchanged probabilities/displays when scored individually and as one batch, and verify both reviewed artifact SHA-256 values remain unchanged.
- [ ] Update source guards so they continue to prohibit the project training CSV and disk/external persistence while allowing only the reviewed in-memory uploaded-CSV helper. Do not weaken guards merely by moving prohibited operations to another module.
...
- [ ] Record implementation, contract decisions, performance/privacy evidence, exact tests, artifact hashes, public verification, and limitations in `docs/p11-batch/report.md`; move P11 to Done only after all closure evidence exists.

#### Expected Tests for P11

- [ ] The generated template and field guide cover exactly `FEATURE_COLUMNS`/`VALUE_RANGES`, use the D-026 encoding, and cannot drift independently from the executable contract.
- [ ] Empty, malformed, wrong-encoding/delimiter, headerless, duplicate-header, missing, unexpected, target, index, identifier, over-byte, over-row, and zero-data-row files fail at the correct boundary with deterministic safe errors.
- [ ] Missing cells, text, booleans, non-finite values, fractions, and out-of-range numbers are reported by row/feature without coercion; all applicable row errors are returned in stable feature order.
- [ ] Structural validity plus mixed row validity scores only valid rows, leaves invalid probabilities blank, preserves all row positions and duplicates, and reports exact valid/invalid totals.
- [ ] Vectorized probabilities equal per-row `predict_risk_probability` results within absolute tolerance `1e-12`, including all four reference profiles; probabilities are finite and within `[0, 1]`.
- [ ] Output columns/order, line endings, encoding, float serialization, blank invalid probabilities, error ordering, and repeated-run bytes are deterministic under D-027.
- [ ] Uploaded bytes and outputs are never written, externally logged, sent to analytics, fetched remotely, or cached across sessions; artifact/upload hash changes and failed processing invalidate stale results.
- [ ] Batch processing performs no fitting, calibration, thresholding, labeling, SHAP, scenario exploration, raw-project-data access, artifact generation, or model/background modification.
- [ ] App/headless coverage verifies progressive batch disclosure, template and result downloads, valid/invalid summaries, bounded preview, limits, reset/replacement, controlled failures, D-019 wording, and disclaimer visibility.
- [ ] The accepted maximum batch passes the D-028 latency/memory limits, the complete suite passes, and mandatory public valid-plus-mixed workflow verification succeeds after deployment.

#### Planned P11 Deliverables
...
- `docs/p11-batch/report.md` with accepted D-026 through D-028 evidence, exact schemas, limits, performance/privacy evidence, tests, artifact hashes, limitations, and public-verification record.
...
- [ ] Do not retrain, recalibrate, retune, replace, or regenerate the D-016/schema-version-2 artifact or P9 background.
- [ ] Do not change the P8 probability contract or D-019 probability-only policy; add no threshold, diagnosis, recommendation, high/low-risk category, or screening interpretation.
- [ ] Do not compute per-row SHAP explanations or scenarios in batch. P9/P10 remain single-case features.
- [ ] Do not use the project raw/training, calibration, or test CSV at runtime; parse only bounded user-uploaded bytes in memory.
- [ ] Do not accept remote URLs, archives, arbitrary identifiers/free text, target labels, or passthrough columns under the candidate contract unless D-026 explicitly replaces that boundary with reviewed evidence.
- [ ] Do not persist, externally log, analyze, or share uploaded profiles or results. No cross-session cache may contain user data.
- [ ] Do not mix P11 with P12 fairness analysis, P13 product polish, CI, `skops`, authentication, general data storage, or user accounts.

#### Definition of Done for P11

- [ ] US-0603, US-0612, and US-0613 satisfy their acceptance criteria.
- [ ] D-026, D-027, and D-028 are Accepted from ordered evidence before their dependent implementation steps.
- [ ] Template, schema, validation, partial-success, scoring, export, resource, privacy, failure, and UI contracts are explicit and reproducible.
- [ ] Every valid batch probability is faithful to the unchanged P8 serving helper, while invalid rows can never receive a probability.
- [ ] No uploaded profile or output is written or externally logged, and the public wording remains probability-only, educational, and non-diagnostic.
- [ ] P9 explanations, P10 scenarios, reference displays, medical disclaimer, and both reviewed artifact hashes remain unchanged.
- [ ] Focused, full-suite, headless, performance, and local visual checks pass.
