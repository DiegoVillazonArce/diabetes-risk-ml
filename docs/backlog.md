...
### Candidate Tasks for P8 (US-0601, US-0606, US-0607)

P5-P7 provide the planning evidence for this refinement: the calibration split has remained untouched since P3 (enforced by poisoned-split tests) and holds exactly 25,368 rows at the preserved ~13.9% prevalence (~3,535 positives), enough for isotonic calibration to be a serious candidate alongside sigmoid; the selected D-016 model's low default-threshold recall (0.157 on test, against 0.563 precision) is the documented concern motivating the threshold analysis; and the serving artifact currently uses schema version 1 with no calibrator and no decision threshold.

#### US-0601: Calibration Evaluation and Selection

- [ ] Record the uncalibrated baseline first: score the calibration split with the frozen train-only D-016 model and report the reliability diagram, Brier score, and log loss, with ROC-AUC and PR-AUC as ranking context, before any calibrator exists.
- [ ] Implement stratified five-fold cross-fitting within the calibration split, operating only on the frozen base model's predicted scores with a fixed random seed: for each method (sigmoid, isotonic), fit the calibrator on four folds, predict the held-out fold, and assemble complete out-of-fold calibrated probabilities. No calibration row ever refits the `HistGradientBoostingClassifier`, and no train or test row ever fits a calibrator.
- [ ] Compare sigmoid, isotonic, and the uncalibrated baseline exclusively on the out-of-fold predictions using the operationalized D-018 criteria declared before results are observed: the paired-bootstrap Brier adoption rule (10,000 fixed-seed resamples; 95% confidence intervals of the mean paired per-row difference `delta = candidate loss - reference loss`, improvement requiring the interval's upper limit below zero), log loss under the same convention as tie-break, the project-defined 0.005 absolute ROC-AUC/PR-AUC regression bound, and sigmoid preferred on demonstrated equivalence; `calibration_method = none` is selected when no method qualifies. Report reliability diagrams as visual diagnostics and ECE as a bin-dependent descriptive metric only, never as deciding criteria.
- [ ] Resolve D-018 with the selected outcome (sigmoid, isotonic, or none) and its recorded evidence, before any threshold freezing, final refit, or test use; do not pre-register any outcome before the comparison runs.
- [ ] Refit the selected calibrator on the full calibration split (skipped when `none` is selected; per-fold calibrators are discarded either way), and record the official P8 test evaluation of the frozen contract, only after the calibration method and the US-0606 threshold scenarios are frozen. Later runs may only repeat that evaluation as a deterministic regression check; never change method or thresholds after observing test results.

#### US-0606: Threshold Analysis and Trade-Offs

- [ ] Build precision-recall curves and threshold tables (positive-class recall, precision, F1, false-positive and false-negative counts, and the confusion matrix per candidate threshold) exclusively from the out-of-fold probabilities of the D-018-selected probability contract on the calibration split.
- [ ] Document the trade-offs against the D-016 baseline concern (default-threshold recall 0.157) in product-neutral language: what each candidate threshold would imply in false negatives and false positives, without clinical claims and without presenting any threshold as a validated screening or diagnostic rule.
- [ ] Freeze the documented threshold scenarios after D-018 is resolved and before the official P8 test evaluation, and resolve D-019 with the scenario set and any product implication.
- [ ] Keep the estimation/decision separation explicit end to end: the app continues to display a probability with no high/low-risk labels unless D-019 records an explicit, justified change, which is not assumed by this plan.

#### US-0607: Schema-Version-2 Artifact and App Integration (final P8 increment)

- [ ] Bump the artifact contract to schema version 2: the bundle stores the frozen D-016 base model, the D-018-selected `calibration_method` (`sigmoid`, `isotonic`, or `none`), and the fitted final calibrator only when the method is sigmoid or isotonic; the metadata records the method, the out-of-fold protocol, the calibration metrics (out-of-fold and the official P8 test evaluation), and the package versions, following the existing single-bundle precedent (an implementation detail within D-010/D-013, not a new decision).
- [ ] Extend `validate_artifact_bundle()` to enforce the version-2 contract as strictly as version 1: a calibrator present if and only if `calibration_method` requires one, direct calibrator-object checks (type, fitted state), calibration metadata completeness, and rejection of version-1 bundles and inconsistent method/calibrator combinations with regeneration guidance.
- [ ] Update the app texts to match the served contract: the disclaimer and the result caption in `app/streamlit_app.py` stop describing the probability as uncalibrated only when a calibrator ships; if D-018 selects `none`, the uncalibrated wording stays. The medical disclaimer is never weakened in either case.
- [ ] Update the four deployment reference profiles through a controlled change of `tests/reference_profiles.py`: when the served probabilities change, recompute each profile's expected probability and display from the new official artifact and re-verify the documented tolerance/rounding margins; when `none` leaves the outputs identical, re-verify the existing expectations against the version-2 artifact. Keep the same 21-feature inputs and boundary ages either way.
- [ ] Verify locally and headlessly (full pytest battery including the reference suite), regenerate and commit the official version-2 artifact per the D-013 controlled Git exception, redeploy, and rerun the public smoke tests with the profiles.
- [ ] Confirm the serving path still never trains, calibrates, downloads models, or reads the raw CSV at runtime.

#### Expected Tests for P8

- [ ] Fit-spy checks: the base model consumes exactly the train rows; each per-fold calibrator is fitted on exactly its four assigned training folds and never on its held-out fold; the final calibrator (when one is selected) is fitted on exactly the full calibration split; no calibrator is ever fitted on train or test rows.
- [ ] Test isolation: test rows never reach the comparison, selection, or threshold-analysis code paths; poisoning the test split must leave the out-of-fold comparison, the D-018 selection, and the threshold tables unchanged. Test enters only the official P8 evaluation and its deterministic regression repeats.
- [ ] Reproducibility: the fixed seed reproduces fold assignment, out-of-fold probabilities, bootstrap intervals, method comparison results, and the selection outcome.
- [ ] Out-of-fold integrity: every calibration row receives exactly one out-of-fold prediction per method, produced by a calibrator that was not fitted on that row's fold.
- [ ] Version-2 artifact contract: save/load round-trip for each `calibration_method` outcome, a calibrator present if and only if the method requires one, rejection of version-1 bundles, tampered or unfitted calibrators, inconsistent method/calibrator combinations, missing calibration metadata, and provenance/runtime pin enforcement carried over from version 1.
- [ ] Controlled reference-profile update: the profiles recompute against the version-2 artifact within the documented tolerance and render the recorded displays through the headless app -- regenerated expectations when the served outputs changed, the existing expectations re-verified otherwise.
- [ ] Source guards: no calibrator fitting or training code in the app serving path; calibration fitting happens only in offline P8 code.

#### P8 Scope Guardrails

- [ ] Do not reopen the D-016 model selection and do not compare new models; P8 calibrates the already selected frozen model only.
- [ ] Never use test rows to select the calibration method or the threshold scenarios; test participates in no P8 decision. One official P8 test evaluation is recorded after method and scenarios are frozen (test's last prior use was the P5 selection protocol); later runs may only repeat it as a deterministic regression check.
- [ ] Never train the base model on calibration rows; never fit a calibrator on train or test rows.
- [ ] No calibration or training inside Streamlit; the app only loads and serves the artifact.
- [ ] Keep SHAP out of P8; P9 owns global and local explainability so explanations can target the final P8 serving contract.
- [ ] Keep fairness analysis, batch prediction, and scenario exploration in their later phases (P10-P12).
- [ ] No medical recommendations, no diagnostic or high/low-risk labels without an explicit D-019 resolution, and no causal interpretation of calibration curves or threshold trade-offs.
