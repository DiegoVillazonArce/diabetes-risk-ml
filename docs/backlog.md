...
### Candidate Tasks for P12 (US-0604, US-0614, US-0615)

P12 audits the already frozen P8 probability contract; it does not reopen model selection, calibration, or the D-019 product policy. Planning evidence may use calibration-split support counts, but not subgroup performance, to make the cohort contract feasible before the official audit. The calibration split supports the proposed single-axis and `Sex x Age` cohorts: each candidate analysis cell has at least 1,011 rows and at least 190 positives, while the proposed four age bands avoid the unstable positive counts of the youngest individual BRFSS codes. These are sufficiency observations only, not P12 fairness results. Because the official audit scores the larger test split (50,736 rows versus calibration's 25,368), these counts are a conservative feasibility signal, and the accepted support floor is still applied mechanically to each test cohort. The calibration support counts, computed via `prepare_data()` and cohort grouping rather than estimated, must be versioned as `docs/p12-fairness/calibration_support.csv` and committed before D-029 is accepted.

#### Increment 1 -- Cohorts, Metrics, and Publication Contract (approximately 1 day)

- [ ] Audit the source semantics and limitations of `Sex`, `Age`, and `Income`. Use the dataset's documented binary `Sex` labels without treating them as a complete representation of sex or gender; preserve all eight ordinal `Income` codes; and evaluate the candidate age mapping `18-49` = codes `1`-`6`, `50-64` = `7`-`9`, `65-74` = `10`-`11`, and `75+` = `12`-`13`.
- [ ] Produce a calibration-only support table for `Sex`, the four age bands, `Income`, and the predeclared `Sex x Age` intersection, generated via `prepare_data()` and cohort grouping and committed as `docs/p12-fairness/calibration_support.csv`; this versioned table is the evidence prerequisite for accepting D-029. Do not calculate or inspect subgroup model-performance metrics during this planning step.
- [ ] Resolve D-029 before the official audit. Evaluate the candidate full-metric rule of at least 500 rows, 100 positive labels, and 100 negative labels in the audited split; unsupported groups must still report support and prevalence with every unavailable metric explicitly marked rather than dropped or merged after results are known.
- [ ] Resolve D-030 before the official audit. Freeze metric definitions, the four D-019 documentation-only scenarios, common reliability-bin edges, gap direction, random seed, bootstrap method/count/confidence level, unavailable-metric behavior, and deterministic output ordering. The candidate uncertainty contract is 5,000 ordinary whole-audit-split row bootstraps with project seed 42 and percentile 95% intervals, recomputing each group and the whole-cohort reference per resample; the resample count must pass a calibration-split runtime benchmark before D-030 is accepted.
- [ ] Resolve D-031 before the official audit. Freeze an all-results publication policy, technical-versus-accessible communication levels, and the default report-only product boundary. Any Streamlit change requires an explicit accepted departure plus headless/public verification; otherwise P12 has no deployment gate.

#### Increment 2 -- Pure Audit Engine and Tests (approximately 1-2 days)

- [ ] Add `src/fairness.py` as a Streamlit-independent engine over explicit targets and served positive-class probabilities. Reuse `FEATURE_COLUMNS`, split contracts, the validated artifact loader/scorer, `FROZEN_THRESHOLD_SCENARIOS`, and project seed rather than duplicating them.
- [ ] Make cohort assignment exhaustive, mutually exclusive, deterministic, and auditable. Preserve source row order internally, but publish no row, target, probability, index, or combination that could expose an individual record.
- [ ] Implement support/prevalence, mean probability, Brier score, log loss, ROC-AUC, PR-AUC, signed calibration gap (`mean_probability - prevalence`), shared-bin reliability data, and recall/precision/FPR/FN/FP for every frozen D-019 scenario. D-019 remains documentation-only and no group-specific threshold may be selected.
- [ ] Implement the accepted fixed-seed bootstrap and group-minus-whole-cohort gaps without significance tests or a fairness pass/fail rule. Degenerate or unsupported metrics must return an explicit unavailable state rather than `NaN` silently propagating or raising an uncontrolled error.
- [ ] Keep evidence generation offline and controlled under `docs/p12-fairness/`. Do not add a fairness package unless a documented compatibility/necessity spike justifies and pins it; the existing NumPy, pandas, scikit-learn, and plotting stack is expected to be sufficient.

#### Increment 3 -- Official P12 Audit and Evidence (approximately 1-2 days)

- [ ] After D-029 through D-031 are Accepted and the engine passes synthetic fixtures, load the official schema-version-2 artifact, score the unchanged P3 test split through the D-018-selected probability contract, and record one official P12 descriptive audit. Test already participated in P5 and P8; do not describe it as pristine or once-only, and do not let P12 results modify any contract or configuration.
- [ ] Export deterministic aggregate support, probability/ranking/calibration, bootstrap-gap, reliability, and frozen-threshold CSVs plus accessible metric-interval and calibration plots. Publish every predeclared eligible cohort regardless of whether the result is favorable.
- [ ] Record exact split provenance, cohort definitions, support decisions, metric formulas, bootstrap configuration, package versions, artifact hashes, reproduction command, runtime, and limitations in `docs/p12-fairness/report.md`.
- [ ] Interpret observed differences alongside uncertainty, prevalence, sample size, and label/access limitations. Do not infer causality, biological mechanisms, discrimination, clinical validity, equalized odds, demographic parity, or universal fairness from the audit.

#### Increment 4 -- Communication, Regression, and Closure (approximately 1 day)

- [ ] Add a concise README summary that links to the technical report and communicates the audit's scope and limitations in everyday language. Keep detailed methods, all aggregates, and plots in GitHub documentation rather than presenting individual subgroup judgments in the prediction UI.
- [ ] Verify deterministic evidence regeneration, exact cohort coverage, absence of real-row outputs, and isolation from train/calibration performance decisions. Re-running the official audit may only reproduce evidence; it cannot change D-029 through D-031 after results are observed.
- [ ] Run focused fairness tests, the complete suite, `pip check`, compile checks, `git diff --check`, artifact-hash checks, and all four reference-profile regressions. If no Streamlit code changes, record that deployment/public smoke testing is not applicable; if D-031 explicitly authorizes an app change, require headless, localhost, and public verification before closure.
- [ ] Update roadmap, backlog, decisions, iteration log, ML plan, and README only after evidence exists. P12 remains Ready and its stories remain unfinished until implementation, review, and every applicable closure gate pass.

#### Expected P12 Tests

- [ ] Cohort mappings are exhaustive, mutually exclusive, correctly labeled, and independent of model results; every audited row belongs to exactly one group per declared axis.
- [ ] Minimum-support behavior is enforced identically for each metric family; unsupported and one-class fixtures expose explicit unavailable metrics while retaining support/prevalence.
- [ ] Metric and D-019 scenario outputs match hand-calculated synthetic fixtures, including confusion counts, signed calibration gap, and group-minus-overall direction.
- [ ] Bootstrap intervals and output bytes are deterministic under the fixed seed; changing the seed changes sampled evidence without changing point estimates.
- [ ] Poisoning train or calibration values after the protocol is frozen cannot change official P12 metrics; test rows never select cohorts, metrics, bins, resampling, thresholds, narrative inclusion, or publication policy.
- [ ] The audit loads and validates the official artifact without fitting, calibrating, threshold-selecting, writing artifacts, or importing Streamlit. Both official SHA-256 values and the four probabilities/displays remain unchanged.
- [ ] Published files contain only declared aggregates and synthetic metadata: no real feature row, target vector, per-row probability, split index, SHAP vector, or user-uploaded content.
- [ ] Documentation and any user-facing copy prohibit causal, clinical, prescriptive, discriminatory, or blanket fair/unfair claims and describe unavailable demographic coverage explicitly.

#### P12 Deliverables

- `src/fairness.py`.
- `tests/test_fairness.py`.
- `docs/p12-fairness/calibration_support.csv`: the pre-audit calibration-only support table (from `prepare_data()` and cohort grouping) committed before D-029 is accepted.
...
- [ ] Do not retrain, reweight, resample training data, recalibrate, regenerate an artifact, compare a new model, optimize a threshold, or implement mitigation in P12.
- [ ] Do not create group-specific thresholds, risk labels, recommendations, diagnoses, or different serving behavior. D-019 remains probability-only.
- [ ] Do not use SHAP importance or local contributions as fairness evidence, and do not reinterpret P9/P10 outputs as causal or intervention effects.
- [ ] Do not hide, merge, rename, or remove a predeclared group after observing official results. Apply the accepted support rule mechanically and publish every eligible result.
- [ ] Do not claim that observed differences prove discrimination or causality, or that small/non-significant differences prove fairness. Confidence intervals are descriptive uncertainty, not certification.
- [ ] Do not generalize beyond the 2015 self-reported BRFSS population or imply coverage of identities absent from the dataset, including race/ethnicity and non-binary sex/gender identities.
- [ ] Do not publish real records, row-level predictions, targets, split indices, or small-cell drill-downs. P12 output is aggregate and offline.
- [ ] Keep P13 product polish, CI, `skops`, authentication, persistence, analytics, and any fairness-mitigation project outside P12.

#### Definition of Done for P12

- [ ] US-0604, US-0614, and US-0615 satisfy their acceptance criteria, and D-029, D-030, and D-031 are Accepted in the required pre-audit order, with D-029 acceptance backed by the committed `calibration_support.csv` evidence.
- [ ] Every predeclared cohort has transparent support/prevalence output; every eligible cohort has the complete probability, ranking, calibration, uncertainty, gap, and frozen-threshold evidence.
- [ ] The official audit is reproducible from the documented command and publishes only aggregates, with exact formulas, seed, versions, split provenance, and artifact hashes.
- [ ] Conclusions communicate differences, uncertainty, base rates, historical/self-reported-label limitations, and missing demographic coverage without causal, clinical, or fairness-certification claims.
- [ ] The model, artifact schema, calibration method, D-019 policy, P9/P10/P11 behavior, both official artifacts, and four reference displays remain unchanged.
- [ ] Focused and full tests, dependency/compile/diff checks, deterministic regeneration, privacy checks, and every applicable UI/deployment gate pass.
- [ ] P12 moves from Ready to Done only after implementation, review, and evidence closure; P13 remains Future.
