...
Operationally, the no-Git-action constraint was satisfied without claiming premature versioning: `calibration_support.csv` was generated and validated first, D-029 was then Accepted in the working tree, and the CSV plus acceptance remain jointly unstaged for the user's eventual commit. D-030 was Accepted only after synthetic checks and the calibration-only 5,000-resample benchmark passed; D-031 was Accepted report-first before official test scoring. The official audit ran only after all three decisions were Accepted. This execution did not modify Streamlit or require deployment verification.
