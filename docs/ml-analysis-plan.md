...
5. Compare the current Git-provenance/recorded-hash/post-load-validator/environment `joblib` boundary with a possible `skops` route, including compatibility, package, schema, artifact, SHAP-background, deployment, and maintenance consequences. Distinguish runtime hash binding from an authenticity allowlist and semantic validation from safe untrusted deserialization. Resolve D-035 without generating or replacing an artifact; migration, if recommended, becomes separate future work.
...
D-035 closes the evaluation promised by D-010 but cannot authorize an artifact migration inside P13. The currently deployed `joblib` bundle is accepted only from the reviewed repository: Git provenance is the root of trust, the recorded/computed hash tracks evidence and binds runtime state/background rather than authenticating the file, and strict schema/object/metadata/environment validation occurs only after deserialization. `skops` would add meaningful defense-in-depth for malicious or tampered files, but changing that chain requires a later phase with its own schema, compatibility, regeneration, SHAP-binding, deployment, and rollback plan.
...
**Execution status (completed 2026-07-20).** All ten rolling-wave steps are complete. D-032 through D-035 were Accepted from audit/spike evidence; the three-section Streamlit navigation with a static About & architecture overview, `docs/architecture.md`, synthetic-only `docs/p13-portfolio/` assets and manifest, `docs/portfolio-summary.md`, README overview and CI badge, and least-privilege `.github/workflows/ci.yml` are implemented and verified. Implementation commit `7f642af` and cross-platform CI fix `08170c3` were pushed; the complete local suite passed 469 tests; clean-clone validation passed 441 with 28 explicit CSV-gated skips; GitHub Actions passed remotely; and the redeployed app passed mandatory public individual/explanation/scenario, valid-plus-mixed batch/download, navigation/About, link, disclaimer, and responsive-layout checks. Both official hashes and the four reference displays remain unchanged, and controlled `joblib` is retained without migration. P13 and Iteration 13 are complete; P0-P13 form the finished portfolio baseline.
