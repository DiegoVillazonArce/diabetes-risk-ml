...
- **Testing:** the current P13 implementation passes 469 local tests; its final clean-clone validation passes 441 tests with 28 explicit CSV-gated skips. P13 must preserve focused, clean-clone, complete local, visual, hash, and public smoke gates while adding no model experiment.
- **Continuous integration:** the D-034 bounded clean-clone workflow is implemented locally with Python 3.12 and no private dataset or credentials, but has not run remotely. A badge can be published only after the accepted workflow passes remotely.
