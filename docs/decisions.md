...
| D-014 | 2026-07-05 | Pending | Decide the duplicate-row handling policy (keep vs. drop) for the raw dataset. | The P2 EDA found 24,206 exact duplicate rows (~9.5%), skewed heavily toward the negative class; dropping them would shift positive prevalence from ~13.9% to ~15.3%. No rows were dropped during EDA. | Required before P3 builds the train/calibration/test split, since the decision changes the prevalence every split must preserve. |
